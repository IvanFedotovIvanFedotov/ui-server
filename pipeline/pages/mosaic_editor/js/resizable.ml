...
(*
  ...
  1.functions for calculate snap if resizing with corner specified:

  let get_item_snap_corner_left_top
  let get_item_snap_corner_left_bottom
  let get_item_snap_corner_right_top
  let get_item_snap_corner_right_bottom*)
...
module Sig : sig
  ...
  let line_find_closest_align_value
    ...
    let rec count_aligns line_align_val distance = function
      ...
      | h1 :: h2 ->
        ...
        let distance =
          if item == h1 
          then count_aligns line_align_val distance h2 (*distance*) else
  ...
  let line_align_count
    ...
    let rec count_aligns line_align_val counts = function
      ...
      | h1 :: h2 ->
        let icompare = Position.of_element h1 in
        if item == h1
        then count_aligns line_align_val counts h2 (*counts*)
        else        
          let counts =
  ...
  let horizontal_lines_aligned_list (item : Dom_html.element Js.t) pos (items : Dom_html.element Js.t list) min_distance =
    let ret_list =
      [ Horizontal_Top,
  ...
  let vertical_lines_aligned_list (item : Dom_html.element Js.t) pos (items:Dom_html.element Js.t list) min_distance =
  ...
  let get_item_snap_y (item : Dom_html.element Js.t) pos min_distance (items : Dom_html.element Js.t list) =
    let snap_list = horizontal_lines_aligned_list item pos items min_distance in
    get_snap item pos.y min_distance snap_list

  let get_item_snap_x (item : Dom_html.element Js.t) pos min_distance (items : Dom_html.element Js.t list) =
    let snap_list = vertical_lines_aligned_list item pos items min_distance in
  ...
  let get_snap_lines
      ...
      (items : Dom_html.element Js.t list)
      min_distance =
    let snap_list_v = vertical_lines_aligned_list item pos items min_distance in
    let snap_list_h = horizontal_lines_aligned_list item pos items min_distance in
