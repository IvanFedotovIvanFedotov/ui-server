  ...
  (direction : resize_direction) =
      ...
      | Top_right ->
          ...
          let snap_list_y = [
            ...
            line_find_closest_align_value item pos items min_distance Horizontal_Top]             
          in
          (*let _ = Printf.printf "top right resize\n" in*)
      ...
      | Bottom_left->
          ...
          let snap_list_y = [
            ...
            line_find_closest_align_value item pos items min_distance Horizontal_Bottom]             
          in
          (*let _ = Printf.printf "bottom left resize\n" in*)
          { x = get_snap item pos.x min_distance snap_list_x 
      ...
      | Bottom_right->
          ...
          let snap_list_y = [
            ...
            line_find_closest_align_value item pos items min_distance Horizontal_Bottom]             
          in
          (*let _ = Printf.printf "bottom right resize\n" in*)
          { x = pos.x (*get_snap item pos.x min_distance snap_list_x *)
          ; y = pos.y (*get_snap item pos.y min_distance snap_list_y *)
          ; w = (get_snap item pos.x min_distance snap_list_x) - pos.x + pos.w
          ; h = (get_snap item pos.y min_distance snap_list_y) - pos.y + pos.h
...
let get_snap_lines_glued_to_item
  (item : Dom_html.element Js.t)
  (pos : t)
  (items : Dom_html.element Js.t list)
  min_distance 
...
let snap_list_v = 
  match action with
    | `Move ->  vertical_lines_aligned_list_for_move_action item pos min_distance items 
    | `Resize direction ->  vertical_lines_aligned_list_for_resize_action item pos min_distance items direction
    in
let snap_list_h = 
  match action with
    | `Move ->  horizontal_lines_aligned_list_for_move_action item pos min_distance items 
    | `Resize direction ->  horizontal_lines_aligned_list_for_resize_action item pos min_distance items direction
    in
let rec create_lines_v_list (action : [`Resize of resize_direction | `Move]) acc = function
  | [] -> acc
...
let rec create_lines_h_list (action : [`Resize of resize_direction | `Move]) acc = function
  | [] -> acc
  | (direction, aligns_count, distance) :: tl ->
    let acc =
      if aligns_count > 0
...
let list_lines_ret =
  ...
  @ create_lines_h_list action [] snap_list_h in
list_lines_ret
...
let is_item_collide_others 
    ...
    (item : Dom_html.element Js.t)
    (items : Dom_html.element Js.t list) =
  let rec _is_item_collide_others (is_collide:bool) (position : t) (item : Dom_html.element Js.t) = function
    | [] -> is_collide
    | hd :: tl ->
...
let position_not_collide_others
    ...
    (item : Dom_html.element Js.t)
    (items : Dom_html.element Js.t list) =
    if is_item_collide_others position item items then 
    begin
...
let adjust ?aspect_ratio
  ...
  in
  let position_snaped = match action with
    | `Move -> get_item_snap_position_for_move_action item position min_distance siblings
    | `Resize resz -> get_item_snap_position_for_resize_action item position min_distance siblings resz
  in
  let last_pos = if !global_saved_original_position <> original_position then
                    begin 
  ...
  let position_not_collide = position_not_collide_others last_pos position_snaped item siblings in
  global_saved_position_previous:=position_not_collide;
  position_not_collide, 
    get_snap_lines_glued_to_item item position_not_collide siblings min_distance action


(*subscribers = ref Int_map.empty*)
