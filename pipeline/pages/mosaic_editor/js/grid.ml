...
module Event = struct
  ...
  let input ?use_capture t = Lwt_js_events.make_event ?use_capture Typ.input t
...
module Util = struct
  let insert_at_idx i x l =
    let rec aux l acc i x =
      ...
      | y :: l' when i = 0 ->
          List.rev_append acc (x :: y :: l')
      | y :: l' ->
          aux l' (y :: acc) (pred i) x
    in
    ...
    Js.Opt.case
      ...
      (fun event ->
        ...
        | Some page_x, Some page_y ->
            (page_x, page_y)
        | _ ->
            failwith "no page coordinates in mouse event")
  ...
  let get_size_at_track ?(gap = 0.) (tracks : float array) =
    ...
    let rec aux sum = function
      | n when n = Array.length tracks ->
          sum
      | n ->
          aux (sum +. tracks.(n)) (succ n)
    in
    aux 0. 0 +. gap
  ...
  let get_cell_position (cell : Dom_html.element Js.t) =
    let parse_span n s =
      ...
      | [s] ->
          int_of_string s - n
      | _ ->
          failwith "unknown cell span value"
    in
  ...
  let set_cell_row ?(span = 1) (row : int) (cell : Dom_html.element Js.t) =
    ...
    cell##setAttribute (Js.string Attr.rowspan)
      (Js.string @@ string_of_int span)
  ...
  let set_cell_col ?(span = 1) (col : int) (cell : Dom_html.element Js.t) =
    ...
    cell##setAttribute (Js.string Attr.colspan)
      (Js.string @@ string_of_int span)
  ...
  let find_first_cell dir n cells =
    ...
    @@ List.fold_left
         (fun acc cell ->
           ...
           match acc with
           | None ->
               if main = n then Some (aux, cell) else None
           | Some (aux', _) ->
    ...
    let rec aux = function
      ...
      | n -> (
        ...
        | None ->
            aux (pred n)
        | Some v ->
            if v <> 0. then Some (n, v) else aux (pred n) )
    in
    ...
    let x, y =
      List.fold_left
        (fun acc v ->
          let c = get_cell_position v in
          let x =
            if c.col + c.col_span > fst acc then c.col + c.col_span
            else fst acc
          in
          let y =
            if c.row + c.row_span > snd acc then c.row + c.row_span
            else snd acc
          in
          (x, y))
        (0, 0) cells
  ...
  let get_after_line_lists (selected_cell : Dom_html.element Js.t)
    ...
    let cross_line =
      generate_check_coords
        ( match what_add with
        | Row ->
            {col= 1; row= 1; col_span= len_x; row_span= 1}
        | Col ->
            {col= 1; row= 1; col_span= 1; row_span= len_y} )
    in
    let check_lines =
      List.fold_left
        (fun acc v ->
          let x =
            generate_check_coords
              ...
              | Row ->
                  if before then
                    {col= fst v; row= 1; col_span= 1; row_span= len_y}
                  else {col= fst v; row= 1; col_span= 1; row_span= len_y + 1}
              | Col ->
                  if before then
                    {col= 1; row= snd v; col_span= len_x; row_span= 1}
                  else {col= 1; row= snd v; col_span= len_x + 1; row_span= 1}
              )
          in
          x :: acc)
        [] (List.rev cross_line)
    in
    let check_lines_with_insert_line =
      List.fold_left
        (fun (acc : ((int * int) list * (int * int)) list * int)
          ...
          let steps =
            List.fold_left
              (fun acc v ->
                ...
                in
                let vcell =
                  get_cell_at_coord ~col:(fst v) ~row:(snd v) cells
                in
                let vc =
                  match vcell with
                  | None -> (
                    ...
                    | Row ->
                        if before then
                          {col= i; row= 1; col_span= 1; row_span= 1}
                        else {col= i; row= len_y + 1; col_span= 1; row_span= 1}
                    | Col ->
                        if before then
                          {col= 1; row= i; col_span= 1; row_span= 1}
                        else {col= len_x + 1; row= i; col_span= 1; row_span= 1}
                    )
                  | Some x ->
                      get_cell_position x
                in
                (* Секция передалана так, чтобы в линии раздела небыло элементов
              ниже (ниже = индекс больше выбранной ячейки), 
              если выбрано before=true
              И также выше если выбрано before=false
              Это важно для работы удаления столбцов, т.к. 
              там тоже используется эта функция *)
                if
                  is_cells_same_at_coords (fst v) (snd v) (fst c) (snd c) cells
                  || not
                       ( match what_add with
                       | Row ->
                           if before then vc.row <= sel.row
                           else vc.row >= sel.row
                       | Col ->
                           if before then vc.col <= sel.col
                           else vc.col >= sel.col )
                then acc
                else v :: acc)
              [] r
          ...
          let steps_sorted =
            List.sort
              (fun a b ->
                ...
                | Row ->
                    let c =
                      if before then sel.row else sel.row + sel.row_span
                    in
                    compare (abs (snd a - c)) (abs (snd b - c))
                | Col ->
                    let c =
                      if before then sel.col else sel.col + sel.col_span
                    in
          ...
          let ret =
            match what_add with
            | Row ->
                if before then (i, 1) else (i, len_y + 1)
            | Col ->
                if before then (1, i) else (i, len_x + 1)
          in
            ...
            (fun acc v ->
              ...
              if
                ...
                | Col ->
                    fst v < fst insln
              then acc
              else match cl with None -> acc | Some cl -> cl :: acc)
            [] cr
          @ acc)
        [] check_lines_with_insert_line
    in
  ...
  let get_delline_addfrags_after_aftershift
      (selected_cell : Dom_html.element Js.t) (what_del : direction)
      (cells : Dom_html.element Js.t list) =
    let len_x, len_y = get_visual_table_size cells in
    let sel_c = get_cell_position selected_cell in
    let after, insertline =
      get_after_line_lists selected_cell what_del true cells
    in
    let delline =
      List.fold_left
        (fun acc v ->
    ...
    let min_v =
      List.fold_left
        (fun acc v ->
          let c = get_cell_position v in
          let len_v =
            match what_del with Row -> c.row_span | Col -> c.col_span
          in
          if len_v < acc then len_v else acc)
        (match what_del with Row -> len_y | Col -> len_x)
        delline
    in
    ...
    let max_v =
      List.fold_left
        (fun acc v ->
          ...
          in
          if len_v > acc then len_v else acc)
        (match what_del with Row -> 1 | Col -> 1)
        delline
    in
    ...
    let del_len =
      List.fold_left
        (fun acc v ->
          let c = get_cell_position v in
          (match what_del with Row -> c.col_span | Col -> c.row_span) + acc)
        0 delline_uniq
    in
    (* Если длина линии не равна всей длине, то это ошибка, 
       тогда ряд/колонку не удаляем *)
    let diff =
      if del_len < match what_del with Row -> len_x | Col -> len_y then 0
          ...
          (fun acc v ->
            ...
            let frag =
              ...
              | Col ->
                  ...
                  ; row_span= c.row_span }
            in
            if frag.col_span <= 0 || frag.row_span <= 0 then acc
            else frag :: acc)
          [] delline
      else []
    in
    ...
    let addfrags_uniq =
      List.sort_uniq
        (fun (pa : cell_position) (pb : cell_position) ->
          if pa.col = pb.col && pa.row = pb.row then 0
          else if pa.col + (pa.row * len_x) > pb.col + (pb.row * len_x) then 1
          else -1)
        addfrags
    ...
    let aftershift =
      match what_del with
      | Row ->
          {col= 0; row= sel_c.row; col_span= 0; row_span= -min_v}
      | Col ->
          {col= sel_c.col; row= 0; col_span= -min_v; row_span= 0}
    in
...
class t ?drag_interval ?(snap_offset = 0.) ?(min_size_start = 20.)
  ...
  object (self)
      ...
      _listeners <-
        Events.
          [ mousedowns super#root (fun e t ->
                match e##.button with
                | 0 ->
                    self#handle_drag_start (e :> Dom_html.event Js.t) t
                | _ ->
                    Lwt.return_unit)
    ...
    method reset ?cells ~(cols : property) ~(rows : property) () =
      ...
      let cells =
        ...
        | None ->
            ...
            Util.gen_cells
              ~f:(fun ~col ~row () ->
                Tyxml_js.To_dom.of_element @@ Markup.create_cell
    ...
    method private remove_row_or_column (direction : direction)
      ...
      let delline, addfrags, after, aftershift =
        (* addfrags, after, aftershift) =*)
        Util.get_delline_addfrags_after_aftershift cell direction _cells
      in
      ...
      List.iter
        (fun cell ->
          ...
          | Row ->
              Util.set_cell_row ~span:row_span (row + aftershift.row_span) cell
          | Col ->
              Util.set_cell_col ~span:col_span (col + aftershift.col_span) cell)
        after ;
      (* Remove elements *)
      ...
      List.iter
        (fun v ->
          ...
          let (elt : Dom_html.element Js.t) =
            Tyxml_js.To_dom.of_element
            @@ Markup.create_cell
                 (make_cell_position ~col ~row ~col_span ~row_span ())
          in
      ...
      let tr_num =
        match direction with
        | Row ->
            List.length tracks + aftershift.row_span
        | Col ->
            List.length tracks + aftershift.col_span
      in
      let style2 =
        if tr_num > 1 then
          let tr2, _ =
            List.fold_left
              (fun acc v ->
                if snd acc <= tr_num then (v :: fst acc, snd acc + 1)
    ...
    method private add_row_or_column ?(size = Fr 1.) ?(before = false)
      ...
      let grid = Util.get_parent_grid cell in
      let tracks = Array.to_list @@ self#raw_tracks grid direction in
      let after_list, line_list =
        Util.get_after_line_lists cell direction before
      ...
      List.iter
        (fun cell ->
          ...
          match direction with
          | Row ->
              Util.set_cell_row ~span:row_span (succ row) cell
          | Col ->
              Util.set_cell_col ~span:col_span (succ col) cell)
        after_list ;
      (* Add elements *)
      List.iter
        (fun v ->
    ...
    method private set_style grid direction (style : string) : unit =
      ...
      match direction with
      | Row ->
          (Js.Unsafe.coerce grid##.style)##.gridTemplateRows := v
      | Col ->
          (Js.Unsafe.coerce grid##.style)##.gridTemplateColumns := v
    ...
    method private update_position grid position (dimensions : dimensions) =
      let position =
        ...
        | Col ->
            float_of_int @@ fst position
        | Row ->
            float_of_int @@ snd position
      in
      let min_position =
      ...
      let position =
        ...
        |> max min_position |> min max_position
      in
      let a_track_size =
        position -. dimensions.a_track_start -. dimensions.gap
      in
      let b_track_size =
        dimensions.b_track_end -. position -. dimensions.gap
      in
      let drag_interval =
        ...
        | None ->
            1.
        | Some (Fr x) ->
            x *. dimensions.fr_to_pixels
        | Some (Px x) ->
            x
        | Some (Pc x) ->
            x *. dimensions.percentage_to_pixels
        | Some Auto ->
            fail "`auto` interval is not supported"
      in
      ...
      let update_value track track_size =
        ...
        | Fr _ -> (
          ...
          | 1 ->
              Fr 1.
          | _ ->
              Fr (track_size /. fr_to_pixels) )
        | Pc _ ->
            Pc (track_size /. percentage_to_pixels)
        | x ->
            x
      in
    ...
    method private track_values_px grid direction : float array =
      ...
      let v =
        match direction with
        | Row ->
            (Js.Unsafe.coerce style)##.gridTemplateRows
        | Col ->
            (Js.Unsafe.coerce style)##.gridTemplateColumns
      in
      Js.Optdef.case v
        (fun () -> [||])
        ( Array.of_list
        % List.map (fun (x : string) ->
              ...
              | Px v ->
                  v
              | _ ->
                  fail @@ Printf.sprintf "failed to parse value (%s)" x)
    ...
    method private gap grid direction : float =
      ...
      let v =
        match direction with
        | Row ->
            (Js.Unsafe.coerce style)##.gridRowGap
        | Col ->
            (Js.Unsafe.coerce style)##.gridColumnGap
      in
      Js.Optdef.case v
        (fun () -> 0.)
        (fun x ->
